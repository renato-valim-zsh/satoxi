defmodule Satoxi.Mnemonic do
  ...
  @lang Application.compile_env(:satoxi, :lang, "en")

  @wordlist :code.priv_dir(:satoxi)
  ...
  @doc """
  Generates and returns a new `t:Satoxi.Mnemonic.t/0` using the given binary.

  The binary entropy must be of a valid `t:Satoxi.Mnemonic.entropy_length/0`.
  """
  ...
  @doc """
  Returns the entropy from the given `t:Satoxi.Mnemonic.t/0`.
  """
  ...
  * `:passphrase` - Optionally secure the seed with an additional passphrase.
  ...
  ## Examples
      ...
      iex> Satoxi.Mnemonic.to_seed("six clarify that goddess door gain stick gentle vault bread taxi champion", encoding: :hex)
      ...
      iex> Satoxi.Mnemonic.to_seed("six clarify that goddess door gain stick gentle vault bread taxi champion", passphrase: "testing", encoding: :hex)
  ...
  defp rand_bytes(bytes) when is_integer(bytes),
